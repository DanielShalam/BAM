"""
Mainly taken from https://github.com/facebookresearch/dino/main/eval_video_segmentation.py
"""
...
def label_propagation(args, model, frame_tar, list_frame_feats, list_segs, mask_neighborhood=None):
    ...
    aff = torch.exp(torch.bmm(feat_tar, feat_sources) / args.t)  # nmb_context x h*w (tar: query) x h*w (source: keys)
...
def extract_feature(model, frame, return_h_w=False):
    ...
    # extract features
    if isinstance(model, vits.ViT):
        out = model.get_intermediate_layers(frame.unsqueeze(0).cuda(), n=1, return_class_token=False, norm=args.norm)[0]
        h, w = int(frame.shape[1] / model.patch_embed.patch_size[0]), int(
            frame.shape[2] / model.patch_embed.patch_size[1])
    else:
        # For DINO models ...
...
if __name__ == '__main__':
    ...
    parser.add_argument("--n_last_frames", type=int, default=7, help="number of preceding frames")
    parser.add_argument("--size_mask_neighborhood", default=12, type=int,
                        help="We restrict the set of source nodes considered "
                             "to a spatial neighborhood of the query node")
    parser.add_argument("--topk", type=int, default=5, help="accumulate label from top k neighbors")
    parser.add_argument("--norm", type=utils.bool_flag, default=True,
                        help="Apply layer-norm on patch representations")
    ...
    # building network
    model = vits.__dict__[args.arch](num_classes=0, patch_size=args.patch_size, dynamic_img_size=True)
    ...
    for i, video_name in enumerate(video_list):
        ...
        print(f'[{i}/{len(video_list)}] Begin to segment video {video_name}.')
